...
- Monthly CAC reporting should default to DTC-only: use Finaloop DTC allocated ad spend divided by Shopify Admin `customersCount` monthly `created_at` counts; for Shopify LTV, use aggregate ShopifyQL customer report data by `customer_added_date`, not individual order/customer walks; upsert and preserve customer counts and `shopify_ltv_to_date` fields across Finaloop KPI rebuilds, exclude Faire/wholesale_faire, company-level spend, and broad sm_expense unless explicitly requested, and use `DELETE ... WHERE true` for `fin_kpi_monthly` rebuilds because the live database rejects DELETE without WHERE.
